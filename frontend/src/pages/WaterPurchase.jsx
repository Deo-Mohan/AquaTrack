...
export default function WaterPurchase() {
  ...
  // ── Water Purchase & P&L Monthly PDF Generator ──────────────────
  const downloadPurchasePnlPdf = () => {
    const timestamp = new Date().toLocaleString('en-IN', { dateStyle: 'full', timeStyle: 'medium' });
    const isProf = (summary?.isProfit ?? true);
    const netPnlVal = summary?.netProfitLoss || 0;
    const profitMargin = summary?.profitMarginPercent || 0;
    const totalWaterPurchased = summary?.totalWaterPurchasedLiters || 0;
    const totalResidentConsumption = summary?.totalResidentConsumptionLiters || 0;
    const totalOutflowCost = summary?.totalPurchaseCost || 0;
    const totalInflowRevenue = summary?.totalResidentBilledRevenue || 0;
    const volumeEfficiency = summary?.volumeEfficiencyPercent || 0;

    let purchaseRowsHTML = '';
    if (purchases.length === 0) {
      purchaseRowsHTML = `<tr><td colspan="7" style="text-align: center; padding: 24px; color: #94a3b8; font-style: italic;">No water purchase entries logged for ${selectedMonth}.</td></tr>`;
    } else {
      purchaseRowsHTML = purchases.map((p, idx) => `
        <tr style="background-color: ${idx % 2 === 0 ? '#ffffff' : '#f8fafc'};">
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-weight: 600;">${p.purchaseDate ? new Date(p.purchaseDate).toLocaleDateString('en-IN') : '—'}</td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-weight: 700; color: #0284c7;">${p.sourceType}</td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0;">${p.vendorName || '—'}</td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-weight: 700;">${(p.volumeLiters || 0).toLocaleString()} L</td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0;">₹${(p.unitCostPerLiter || 0).toFixed(3)}</td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-weight: 800; color: #e11d48;">₹${(p.totalCost || 0).toFixed(2)}</td>
          <td style="padding: 10px 12px; border-bottom: 1px solid #e2e8f0; font-size: 11px; color: #64748b;">${p.notes || '—'}</td>
        </tr>
      `).join('');
    }
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .kpi-box { flex: 1; padding: 14px; border-radius: 12px; border: 1px solid #e2e8f0; }
            .kpi-title { font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; color: #64748b; }
            .kpi-value { font-size: 20px; font-weight: 900; margin-top: 4px; }
            .kpi-sub { font-size: 11px; font-weight: 700; margin-top: 2px; }
        ...
        <body>
          <div class="header-bar">
            <div>
              <div class="logo-title">💧 AquaTrack Water Management</div>
              <div style="font-size: 16px; font-weight: 800; color: #334155; margin-top: 4px;">Water Purchase & Profit & Loss Statement</div>
              <div class="meta-text">Billing Month: <strong>${selectedMonth}</strong> · ${apartmentBlock} · Generated by Community Admin (${username || 'Admin'})</div>
            </div>
          ...
          <!-- KPI Summary Financial Grid -->
          <div class="kpi-grid">
            <div class="kpi-box" style="background-color: #fffbeb; border-color: #fde68a;">
              <div class="kpi-title" style="color: #b45309;">Water Purchased</div>
              <div class="kpi-value" style="color: #d97706;">${totalWaterPurchased.toLocaleString()} L</div>
              <div class="kpi-sub" style="color: #b45309;">Total Cost: ₹${totalOutflowCost.toFixed(2)}</div>
            </div>

            <div class="kpi-box" style="background-color: #f0f9ff; border-color: #bae6fd;">
              <div class="kpi-title" style="color: #0369a1;">Resident Consumption</div>
              <div class="kpi-value" style="color: #0284c7;">${totalResidentConsumption.toLocaleString()} L</div>
              <div class="kpi-sub" style="color: #0369a1;">Billed Revenue: ₹${totalInflowRevenue.toFixed(2)}</div>
            </div>

            <div class="kpi-box" style="background-color: ${isProf ? '#ecfdf5' : '#fef2f2'}; border-color: ${isProf ? '#a7f3d0' : '#fecaca'};">
              <div class="kpi-title" style="color: ${isProf ? '#047857' : '#b91c1c'};">Net P&L Result (${isProf ? 'PROFIT 📈' : 'DEFICIT 📉'})</div>
              <div class="kpi-value" style="color: ${isProf ? '#059669' : '#dc2626'};">${isProf ? '+' : ''}₹${netPnlVal.toFixed(2)}</div>
              <div class="kpi-sub" style="color: ${isProf ? '#047857' : '#b91c1c'};">Profit Margin: ${profitMargin}%</div>
            </div>

            <div class="kpi-box" style="background-color: #faf5ff; border-color: #e9d5ff;">
              <div class="kpi-title" style="color: #7e22ce;">Volume Efficiency</div>
              <div class="kpi-value" style="color: #9333ea;">${volumeEfficiency}%</div>
              <div class="kpi-sub" style="color: #7e22ce;">Purchased vs Billed Ratio</div>
            </div>
          </div>

          <!-- Purchase Logs Detailed Table -->
          <div style="margin-top: 20px;">
            <div style="font-size: 14px; font-weight: 800; color: #0f172a; margin-bottom: 8px;">
              📦 Itemized Water Purchase Logs (${selectedMonth})
            </div>
            <table>
              <thead>
                <tr>
          ...
          <div class="footer">
            AquaTrack Water & Infrastructure Monitoring Platform — Official Monthly Water Acquisition & Financial P&L Audit Statement (${selectedMonth})
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2">
            <h1 className="text-xl sm:text-2xl font-bold text-text">Water Purchase & P&L</h1>
            <span className="w-fit px-2.5 sm:px-3 py-1 rounded-full bg-emerald-500/15 dark:bg-emerald-500/20 border border-emerald-600/30 dark:border-emerald-400/30 text-emerald-700 dark:text-emerald-300 text-[11px] sm:text-xs font-bold flex items-center gap-1.5 shadow-sm whitespace-nowrap">
              <Lock className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
              <span>Locked Resident Billing</span>
            </span>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2 sm:gap-2.5 w-full md:w-auto">
          {/* Billing Month Selector */}
          <div className="flex items-center justify-between gap-2 bg-surface-lighter px-3 py-2 rounded-xl border border-border shrink-0">
            <div className="flex items-center gap-2 w-full justify-between md:justify-start">
              <div className="flex items-center gap-2">
                <Calendar className="w-4 h-4 text-primary shrink-0" />
                <span className="text-[11px] uppercase font-extrabold text-text-muted md:hidden">Period:</span>
              </div>
              <select
                value={selectedMonth}
          ...
          <div className="grid grid-cols-2 md:flex items-center gap-2 w-full md:w-auto">
            {/* Export Monthly P&L PDF Report Button */}
            <button
              onClick={downloadPurchasePnlPdf}
              className="flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white rounded-xl text-xs font-bold shadow-md shadow-emerald-600/20 transition-all cursor-pointer border border-emerald-500/40 whitespace-nowrap"
              title={`Export P&L PDF Statement for ${selectedMonth}`}
            >
              <FileText className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
              <span className="hidden sm:inline">Export P&L PDF</span>
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 active:scale-[0.98] text-white rounded-xl text-xs font-bold shadow-md shadow-blue-600/20 transition-all cursor-pointer whitespace-nowrap"
            >
