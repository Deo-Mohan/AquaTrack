...
export default function SmartSearchBar({
  ...
  return (
    <div className="space-y-2 w-full">
      {/* ── Desktop & Mobile Unified Control Row ── */}
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2.5 w-full">
        {/* Search Input (Takes remaining flexible space on desktop) */}
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
          ...
          <button
            ...
            title={`Sort (${SORT_OPTIONS.find(o => o.value === sortField)?.label ?? 'Default'}): ${sortDir === 'asc' ? 'A → Z' : 'Z → A'}`}
            className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl border border-border bg-surface text-text-muted hover:text-text hover:border-primary/30 transition-all text-xs sm:text-sm cursor-pointer whitespace-nowrap shrink-0"
          >
            {sortDir === 'asc'
              ? <ChevronUp className="w-4 h-4 text-primary shrink-0" />
              : <ChevronDown className="w-4 h-4 text-primary shrink-0" />}
            <span className="text-xs font-bold text-text hidden sm:inline">
          ...
          <div className="relative shrink-0">
            <button
              ...
              title="Sort & Filter"
              className={`relative flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border text-xs sm:text-sm font-bold transition-all cursor-pointer whitespace-nowrap ${
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-2.5 sm:px-3 py-2.5 rounded-xl border border-red-500/30 text-red-400 bg-red-500/5 hover:bg-red-500/15 text-xs sm:text-sm font-medium transition-colors cursor-pointer whitespace-nowrap shrink-0"
            >
