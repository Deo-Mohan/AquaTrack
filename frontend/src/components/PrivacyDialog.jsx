...
export default function PrivacyDialog() {
  ...
  useEffect(() => {
    ...
    if (!accepted) {
      // Small delay before showing the dialog
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-6 right-6 z-[100] w-[90%] sm:w-80 h-auto rounded-2xl bg-surface border border-border shadow-2xl p-6"
        >
          <div className="flex flex-col items-center justify-between relative">
            <span className="relative mx-auto -mt-14 mb-6 bg-surface-lighter p-2 rounded-full border border-border">
              <svg
            ...
            <h5 className="text-sm font-semibold mb-2 text-left mr-auto text-text">
              Your privacy is important to us
            </h5>

            <p className="w-full mb-4 text-xs text-text-muted leading-relaxed text-justify">
              ...
              <span className="block mt-1">
                ...
                <a
                  href="#"
                  className="text-xs cursor-pointer font-semibold transition-colors hover:text-primary underline underline-offset-2"
                >
                  Privacy Policy
                </a>
              </span>
            ...
            <div className="flex items-center justify-between w-full mt-2">
              <button
                type="button"
                className="text-xs text-text-muted cursor-pointer font-semibold transition-colors hover:text-primary hover:underline underline-offset-2"
              >
                More Options
              </button>
              <button
                onClick={handleAccept}
                className="font-semibold cursor-pointer py-1.5 px-6 w-max break-keep text-xs rounded-xl transition-colors text-white bg-primary hover:bg-primary-dark"
                type="button"
              >
                Accept
              </button>
